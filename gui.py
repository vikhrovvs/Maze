...
def relax_loop():
    ...
    color = random.choice((LIGHT_BLUE, GREEN, YELLOW, PINK))
    x_start, y_start = random.randint(n / 4, 3 * n / 4), random.randint(n / 4, 3 * n / 4)
        ...
        for event in pygame.event.get():
                    ...
                    for i, j in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                        ...
                        if wall is not None and maze.walls[wall] == False and maze.walkthrough[x + i][y + j] > 0:
                            ...
                            if maze.walkthrough[x][y] == FINISH:
                                ...
                                leaderboard.write(str(score) + ', ' + str(time) + ', ' + str(maze.hints) + ', ' + str(excess_turns) + '\n')
                                leaderboard.close()
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_x:  # relax
                    if relax_loop():  # if ended with QUIT
                        return
                    finished = False
                    start_time = datetime.datetime.now()
                    maze.create_random_maze()
                if event.key == pygame.K_l: # show best result
                    if finished and not show_highscore:
                        ...
                        leaderboard.close()
                        draw_maze(screen, 0, 0, grid_size, maze, n, m)
                        blit_text(screen, 'Highscore:\n' + get_result_text(highscore, h_time, h_hints, int(h_excess_turns)))
                        pygame.display.flip()
                    elif show_highscore:
                        show_highscore = False
...
pygame.init()
screen: Surface = pygame.display.set_mode([960, 960])
game_loop(screen)
pygame.quit()
